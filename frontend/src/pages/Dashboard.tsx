    ...
    const fetchDashboardData = async () => {
      try {
        const [postsRes, accountRes, activityRes] = await Promise.all([
          api.get(ENDPOINTS.posts.base),
          api.get(ENDPOINTS.accounts.base),
          api.get(ENDPOINTS.activity.base),
        ]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {statCard.map((card) => (
          <div
            key={card.label}
            className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5 hover:border-red-50 transition-all"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        {activities.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-6">
            ...
            <p className="text-slate-500">No acivity yet</p>
