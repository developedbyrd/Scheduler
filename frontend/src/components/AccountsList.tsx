...
const AccountsList = ({ accounts, onDisconnect }: AccountListProps) => {
  const handleDisconnect = async (accountId: string) => {
    const confirm = window.confirm(
      "Are you sure you to disconnect this account?",
    );
    if (!confirm) return;
  ...
  if (accounts.length === 0)
    return (
      <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center py-20 px-6">
        <div className="size-14 bg-slate-50 rounded-2xl flex items-center justify-center mb4 border-slate-100">
        ...
        return (
          ...
          >
            ...
            <button
              ...
              className="ml-2 p1.5
             rounded-lg text-slate-300 group-hover:text-red-500 transition-all cursor-pointer"
            >
